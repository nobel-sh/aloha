...
namespace aloha
{
  Parser::Parser(Lexer &lexer, TySpecArena &arena, DiagnosticEngine &diag)
      : lexer(&lexer),
  ...
  {
    diagnostics.error(DiagnosticPhase::Parser, peek()->loc, message);
  }
  ...
  {
    auto program = std::make_unique<ast::Program>(current_location());
  ...
  std::unique_ptr<ast::Expression> Parser::parse_struct_field_access()
  {
    ...
    return std::make_unique<ast::StructFieldAccess>(loc, std::move(struct_name),
                                                    std::move(field_name));
  }
  ...
  std::unique_ptr<ast::Statement> Parser::parse_expression_statement()
  {
    ...
    return std::make_unique<ast::ExpressionStatement>(loc, std::move(expr));
  }

  std::unique_ptr<ast::StatementBlock> Parser::parse_statements()
  {
    Location loc = current_location();
  ...
  std::unique_ptr<ast::Statement> Parser::parse_variable_assignment()
  {
    Location loc = current_location();
    auto identifier = expect_identifier();
  ...
  std::unique_ptr<ast::Statement> Parser::parse_struct_field_assignment()
  {
    ...
    return std::make_unique<ast::StructFieldAssignment>(
  ...
  std::unique_ptr<ast::Statement> Parser::parse_if_statement()
  {
    ...
    std::unique_ptr<ast::StatementBlock> then_branch = parse_statements();
    std::unique_ptr<ast::StatementBlock> else_branch = nullptr;
    if (match("else"))
    {
      advance();
      if (match("if"))
      {
        std::vector<ast::StmtPtr> else_stmts;
        loc = current_location();
        else_stmts.push_back(parse_if_statement());
        else_branch =
            std::make_unique<ast::StatementBlock>(loc, std::move(else_stmts));
      }
      else
      {
  ...
  std::unique_ptr<ast::Statement> Parser::parse_while_loop()
  {
    ...
    return std::make_unique<ast::WhileLoop>(loc, std::move(condition),
                                            std::move(body));
  }
  ...
  std::unique_ptr<ast::Expression> Parser::parse_primary()
  {
    ...
    {
      advance();
      return std::make_unique<ast::Integer>(loc, std::stoll(token->get_lexeme()));
    }
    if (match(TokenKind::FLOAT))
    {
      advance();
      return std::make_unique<ast::Float>(loc, std::stod(token->get_lexeme()));
    }

    if (match(TokenKind::STRING))
    {
      advance();
      return std::make_unique<ast::String>(loc, token->get_lexeme());
    }
  ...
  std::unique_ptr<ast::Expression> Parser::parse_array_access()
  {
    ...
    return std::make_unique<ast::ArrayAccess>(loc, std::move(array_expr), std::move(index_expr));
  }
  ...
  std::unique_ptr<ast::Expression> Parser::parse_array()
  {
    Location loc = current_location();
    ...
    return std::make_unique<ast::Array>(loc, std::move(members));
  }

  std::unique_ptr<ast::Identifier> Parser::expect_identifier()
  {
    Location loc = current_location();
    if (match(TokenKind::IDENT))
    {
  ...
  ParseTy Parser::parse_type()
  {
    ...
    if (match(TokenKind::IDENT))
    {
      ...
      if (lexeme == "int")
      {
        return type_arena->builtin(loc, TySpec::Builtin::Int);
      }
      else if (lexeme == "float")
      {
        return type_arena->builtin(loc, TySpec::Builtin::Float);
      }
      else if (lexeme == "bool")
      {
        return type_arena->builtin(loc, TySpec::Builtin::Bool);
      }
      else if (lexeme == "string")
      {
        return type_arena->builtin(loc, TySpec::Builtin::String);
      }
      else if (lexeme == "void")
      {
        return type_arena->builtin(loc, TySpec::Builtin::Void);
      }
      else
      {
  ...
  void Parser::dump(ast::Program *p, const TySpecArena &arena) const { p->write(std::cout, arena, 2); }
} // namespace aloha
